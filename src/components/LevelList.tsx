...
function levelPassed(progress: ProgressState, level: LevelDef, PASSING = 70) {
  return level.sublevels.every((sub) => (progress[sub.id]?.score ?? 0) >= PASSING);
}
function countTopLevelsPassed(progress: ProgressState, levels: LevelDef[], PASSING = 70) {
  return levels.reduce((acc, lvl) => (levelPassed(progress, lvl, PASSING) ? acc + 1 : acc), 0);
}
...
export default function LevelList({
  ...
  const completedTopLevels = countTopLevelsPassed(progress, levels, PASSING); // How many top levels are finished
            ...
            {expanded[lvl.key] && !locked && (
              <View style={{ marginTop: -6, marginBottom: 8 }}>
                {lvl.sublevels.map((s, idx) => {
                  ...
                  const subUnlocked = isSubLevelUnlocked(lvl, idx, progress, PASSING);
                  ...
                  return (
                    <View
                      ...
                      <TouchableOpacity
                        ...
                        hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
                        <Image
                          source={require('../../assets/arrow.png')} // adjust to your asset path
...
const styles = StyleSheet.create({
  ...
  progressHeader: {
    flexDirection: 'row',
    justifyContent: 'space-between', // text left, dots right
    alignItems: 'center',
  },
  progressTitle: { fontWeight: 'bold', fontSize: 16 },
  progressText: { fontSize: 14, color: '#666' },
  progressDots: { flexDirection: 'row', marginLeft: 10 },
  dot: {
    width: 10,
    height: 10,
    borderRadius: 5,
